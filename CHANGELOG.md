...
- **贡献热力图改用 GitHub 配色**：四阶绿色取自 GitHub 现行配色（改版后的 `#aceebb` / `#4ac26b` / `#2da44e` / `#116329`，不是流传更广的 2022 年前旧值），方块尺寸收窄。Coding 页与归档页的两张热力图统一到同一套颜色。
- **Coding 页新增语言占比**：热力图右侧一张环形图，按仓库数统计，颜色沿用 GitHub 的语言色；第 6 名之后并入「Other」，不做静默截断。
