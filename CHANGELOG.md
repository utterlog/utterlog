...
- **Azure 主题侧栏认出回访读者**：评论过的访客再次来访时，侧栏整块换成他自己的信息——头像、昵称、等级与累计评论数，等级徽章挂在头像右下角；此时不再显示博主的头像、简介和社交链接。识别依据是浏览器本地缓存的评论邮箱，未评论过或清空缓存则照常显示博主信息。

### 移除

- **Coding 页面整体下线**：`/coding` 路由、GitHub 数据抓取接口、后台「配置 Coding」面板、五个 `coding_*` 配置项、专属样式与三语言文案全部移除。升级时会自动清理数据库里的遗留配置行，并把导航菜单里指向 `/coding` 的条目摘掉（其余菜单项不受影响）。「设置 → 第三方服务」的 GitHub Token 字段**保留** —— 它同时服务于版本更新检查，不是 Coding 页专用。

### 修复

- **Mapbox 的「测试」按钮会给出假阳性**：它此前只检查 Token 是否存在，不验证能否真的调用地理编码。于是出现「后台显示有效、说说定位却一直失败」的情况。现在改为发起一次真实的逆地理编码请求，权限不足或额度用尽会明确报出原因。
- **定位失败时服务端不再无声无息**：三个地图服务全部失败时只返回空结果，日志里没有任何线索。现在会记录每一家的具体失败原因。


- **后台的「像素化」图片效果此前没有实现**：选中它等同于关闭效果。现已替换为「马赛克」——首页 banner 换图时方块逐个化开，露出图片。揭示方向有八种（斜扫、左右推、中心扩散、打散等），每张图按自身固定选一种，看久了不会腻。


- **发布新版本会让正在浏览的读者页面卡住**：站点是单页应用，读者打开页面时已经确定了要加载哪些资源文件，而发布时旧文件被直接删除，他们再点导航就会报「加载失败」且无法恢复。现在旧资源保留三天，并且万一仍然失效会自动重新加载一次。


- **「友链更新」标签页永远点不开**：使用 Flux / Utterlog 等经典主题时，文章底部导航的这个标签被固定禁用，即使友链有更新也点不进去。


- **友链页时间显示导致的页面反复重渲染**：「最后更新」的日期在服务端按服务器时区、在浏览器按读者时区各算一次，跨日的时间戳两边差一天，触发 React hydration 失败。现在统一按站点时区计算，服务端与浏览器输出一致。


- **页面切换过渡引起的无限闪烁**：上一版加的 View Transitions 与 hydration 冲突，`/links` 等页面反复重渲染、内容出不来。已回滚该效果，页面切换的进度条与加载转圈不受影响。


- **页面切换不再出现无反馈的空白**：切换过程原本是「停在旧页面几百毫秒 → 整块空白只剩边框 → 内容出现」，全程没有任何加载提示。现在顶部进度条覆盖数据加载阶段、页面中央的转圈覆盖组件加载阶段，两段首尾相接。转圈延迟 250ms 才淡入，快的切换看不到它。

### 优化

- **顶部导航栏改为半透明磨砂**：滚动时内容会从导航栏底下透出来，不再是一条压在页面上的实心白板。二级下拉菜单同样改为磨砂，与导航栏观感一致。


- **切换页面时盖一层全屏磨砂**：底下的页面仍看得见轮廓，中间四个方块提示正在加载，切换完成后淡出。加载期间不接受点击，避免连点导航互相打断。


- **首页 banner 不再轮播「最新文章」**：下方文章列表本就按时间倒序，第一条永远是最新那篇，banner 再放一次等于同一篇占了首屏两个位置。现在只轮播热门 / 热评 / 随机。


- **切换页面时不再有空白**：页面组件此前要等切换发生才开始下载，导致框架先出现、内容区空着。现在取数据的同时并行下载组件、首屏空闲后预取全部页面，并把取回的组件缓存下来直接复用——实测切换全程零空白帧。万一仍需等待，占位也从「一个转圈」换成了骨架屏，能看出版面结构。
- **首页 banner 换图用竖条加载指示**：块状区域用三条起伏的竖条，比圆环更压得住；其余位置仍是圆环。


- **全站加载指示换新**：图片加载、评论提交、验证码、翻页等处统一成一段沿圆周伸缩的弧线，此前散落在各处的旧样式一并收敛。
- **字体不再迟到**：三个字体样式表原先要等主样式下载解析完才开始加载（晚 630 毫秒），文字会先用系统字体显示再闪一下。现在与其余样式同时加载。
- **页面切换恢复交叉淡化**：此前因故障回滚，根因（时间显示导致的渲染不一致）修复后重新启用。


- **首页翻页不再像刷新**：此前点分页会把文章列表整块换成「加载中」并滚回顶部。现在旧文章留在原地淡出、新文章淡入，滚动位置不动。


- **头像改为懒加载**：友链页 36 张站点图标此前全部在首屏预加载，评论头像同理。现在滚动到可见范围才加载。


- **文章页体积减少三分之二**：代码高亮改用常用语言集（原先打包了 200 多种语言的语法定义），含代码块的文章页 JS 从 1535 KB 降到 510 KB。全站文章实际只用到 bash / html / css / json / yaml / php 等，冷门语言仍会显示，只是不高亮。
- **地图样式不再拖累全站**：mapbox 的 39.8 KB 样式表原先挂在全局，每个页面都要为它付阻塞渲染的代价，现在跟着地图组件走，只有足迹页加载。


- **头像抽成公共组件**：评论区头像原先在四份主题副本里各写了三遍、共十二处逐字符相同的代码，改尺寸或兜底逻辑要改十二个地方。现在统一走 `<Avatar>`，兜底图和加载失败处理只在一处维护。


- **后台刷新友链 RSS 的提示写清单位**：原文「已刷新 N 个订阅，新增 M 条」两个单位挨在一起，容易把抓回的文章数误读成友链数。改为「已刷新 N 个订阅源，抓回 M 篇文章」。


- **头像统一取 256px**：此前站内 Gravatar 请求的尺寸有 40 / 44 / 48 / 64 / 80 / 96 / 112 / 128 八种，高分屏下明显发糊。现在全部取 256px，由 CSS 控制显示大小。


- **页面切换加上过渡动画**：切换时浏览器原生做交叉淡化（View Transitions），内容就绪后再淡入上移。两段各管一跳、不叠加。不支持的浏览器自动退回瞬间切换；系统开启「减弱动态效果」时全部关闭。

- **友链页改成一行一个**：每行是「站点图标 · 站点名 · 描述 · 最后更新」，描述超长省略、悬停看全文；窄屏隐藏描述保留站名和时间。
- **友链显示各站最后更新时间**：取自该站 RSS 已抓回的最新条目（30 天内显示「3 天前」，更早显示具体日期）。数据来自现有的订阅抓取，不新增任何外部请求；没填 RSS 的友链留空不显示。
- **页脚 RSS 图标点击后变绿色对号**：复制成功的反馈从只有一闪而过的 toast，变成图标本身停留 2 秒的对号。


- **归档页贡献热力图改用 GitHub 配色**：四阶绿色取自 GitHub 现行配色（改版后的 `#aceebb` / `#4ac26b` / `#2da44e` / `#116329`，不是流传更广的 2022 年前旧值）。

- **滚动显现动画**：首页文章卡片、正文里的图片与代码块、评论列表在滚入视口时淡入上移。首屏已可见的内容不做入场动画；系统开启「减弱动态效果」时自动关闭。
- **文章目录悬停显示完整标题**：目录里被省略号截断的长标题，鼠标悬停可看全文；条目文字与左侧竖线之间补上间距，一级标题不再紧贴竖线。
- **Azure 首页 Banner 标题前显示分类图标**：轮播的文章标题左侧会带上所属分类的图标，用分类自己配置的图标，未配置的按分类名兜底。
- **Azure 首页 Banner 改为通栏**：移除 Banner 旁的分类标签（分类入口在文章卡片和存档页都有），Banner 独占整行，可按标准横幅比例出图（当前约 3.5:1）。此前 Banner 高度随分类数变化（分类增减会让它忽高忽低），现已固定。Banner 仍会在最新／热门／热评／随机之间自动轮播。
- **页面切换时不再白屏**：点开文章后到内容出现之间会短暂空白，现在顶部会出现一条细进度条。200 毫秒内完成的跳转不显示，避免一闪而过反而显得卡顿。
- **访客卡片新增成就徽章**：沙发次数、文章覆盖率、陪伴年数，达到条件才出现；悬停头像可看完整履历（等级、评论数、覆盖篇数、首次与最近评论时间）。覆盖率按「评论过的文章数 ÷ 站点文章数」计算，在同一篇下连发多条不会虚增。
- **页脚在线访客只显示人数**：点击展开的访客明细列表已移除——它会把每位在线访客的昵称、脱敏 IP、所在城市和**当前正在浏览的页面**公开给所有人。接口相应精简，不再为每位访客做 2-3 次数据库查询。

### 修复

- **Coding 页混入了关注者的动态**：动态列表里会出现你在 GitHub 上关注的人的提交，把自己的记录挤掉。这个行为默认开启且后台没有开关，站长既没开过也关不掉。现已默认关闭，并在「页面 → Coding」新增开关。


- **分类 / 标签 / 影视的详情页显示成了列表页**：访问某个分类（如 `/categories/trade`）时，页面显示的是全部分类的列表而不是该分类下的文章，标签和影视详情页同样如此。

- **博主评论会暴露常驻位置**：博主的每条评论都显示归属城市，等于公开自己的行踪。现在对访客隐藏博主的地理位置（后台仍可查看），访客自己的归属地不受影响。
- **评论者等级和累计评论数一直是假的**：所有人都显示 Lv.1、累计 1 条评论——这两个值在代码里是写死的常量，从未真正统计过。现在按邮箱（登录用户按账号）汇总实际已通过的评论数换算等级，10 档门槛为 1/3/6/10/20/35/60/100/160/250 条。
- **Azure 主题文章页侧边目录没有样式**：目录相关的样式类在全局样式里没有定义，Azure 也没有自己写，导致目录只剩一列没有容器和层级的链接。现在补齐卡片边框、标题栏、限高滚动与当前位置高亮。

### 新增

- **Renascent 主题首页 Hero 重做**：左栏为常驻城市、大标题（可用 `*星号*` 标记斜体强调词）、简介与档案表；右栏是一张 DISPATCH 卡片，含世界地图、常驻地列表、最新动态条目与版本信息。城市、档案表、项目列表在「主题 → 首页 Hero」里配置，留空的板块不显示，未配置时退化为「大标题 + 简介」，仍是完整版面。世界地图为内联绘制，不依赖外部地图库。

### 优化

- **Renascent 主题**：文章页正文顶部不再重复显示 AI 摘要（标题下方已有同样内容）；正文区右侧不再固定空出一块，文字行宽限制在最佳阅读宽度，图片与代码块可铺满整栏；字号统一到设计规范的刻度上；弹层与卡片阴影收敛到规范层级；正文链接补上悬停反馈。

### 修复

- **Coding 页选中的组织仓库不显示**：后台选了 10 个仓库前台只出现 8 个，且没有任何提示。仓库列表只从个人名下拉取，不包含组织仓库，选中的组织项目在过滤时被静默丢弃——恰好是最想展示的两个项目。现在会单独补齐这类仓库，拉取失败也会记入日志。
