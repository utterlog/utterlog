  ...
  useEffect(() => {
    ...
    const finish = () => {
      ...
      setTimeout(() => {
        ...
        // 给浏览器一帧切 src + key，再淡出 spinner，否则 spinner 消失时
        // 新 img 还没贴上 dom 会闪一下旧图。
        //
        // **rAF 必须带 setTimeout 兜底**：隐藏标签页里 requestAnimationFrame
        // 根本不触发，只靠它的话 setHeroLoading(false) 永远不执行 —— 后台打开
        // 首页（cmd+点击、「在新标签页打开」）时 hero 会一直卡在模糊 + spinner
