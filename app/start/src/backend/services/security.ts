...
export async function getSecuritySettings() {
  ...
  return {
    ...
    geo_countries: countries.split(',').map((value) => value.trim()).filter(Boolean),
...
export async function securityOverview() {
  ...
  const [settings, totalBans, activeBans, totalEvents, events24h] = await Promise.all([
    ...
    one<{ count: string }>(`select count(*)::text as count from ${table('security_events')} where created_at >= $1`, [now - 86400]).catch(() => null),
  ]);
  return { total_bans: Number(totalBans?.count || 0), active_bans: Number(activeBans?.count || 0),
    total_events: Number(totalEvents?.count || 0), events_24h: Number(events24h?.count || 0),
    cc_enabled: settings.cc_enabled, geo_enabled: settings.geo_enabled };
}

export async function updateSecuritySettings(body: Record<string, unknown>) {
...
export async function listSecurityBans() {
  await exec(`delete from ${table('ip_bans')} where expires_at > 0 and expires_at < $1`, [nowUnix()]).catch(() => {});
  return many<Record<string, unknown>>(`select * from ${table('ip_bans')} order by created_at desc`);
}

export async function banIp(body: Record<string, unknown>) {
  const ip = String(body.ip || '').trim();
  if (!ip) throw new SecurityServiceError(400, 'VALIDATION_ERROR', 'IP 不能为空');
  ...
  await exec(
    `insert into ${table('ip_bans')} (ip, reason, ban_type, duration, expires_at, created_at)
     values ($1,$2,'manual',$3,$4,$5) on conflict (ip) do update set reason=$2, duration=$3, expires_at=$4`,
...
export async function unbanIp(body: Record<string, unknown>) {
  const ip = String(body.ip || '').trim();
  if (!ip) throw new SecurityServiceError(400, 'VALIDATION_ERROR', 'IP 不能为空');
...
export async function securityTimeline(options: { ip?: string; page?: number; perPage?: number; paginated?: boolean }) {
  if (!options.paginated) {
    const rows = await many<Record<string, unknown>>(
      `select * from ${table('security_events')} order by created_at desc, id desc limit 200`,
    ).catch(() => []);
    return { rows, meta: null };
  }
  const page = Math.max(1, Math.floor(options.page || 1));
  const perPage = Math.min(500, Math.max(1, Math.floor(options.perPage || 50)));
  const ip = String(options.ip || '').trim();
  const where = ip ? 'where ip = $1' : '';
  const params: unknown[] = ip ? [ip] : [];
  const totalRow = await one<{ count: string }>(`select count(*)::text as count from ${table('security_events')} ${where}`, params);
  const rows = await many<Record<string, unknown>>(
    `select * from ${table('security_events')} ${where} order by created_at desc, id desc
